		...
		if (!empty($_POST)) {
			...
			if (count($enquiryFormErrors) > 0) {
				...
				if (isset ($_POST["submitExpressEnquiry"])) {
					$customerName = html_entity_decode($_POST['customerName']);
					$sourceLanguage = html_entity_decode($_POST['sourceLanguage']);
					$targetLanguage = html_entity_decode($_POST['targetLanguage']);
...
Diese E-Mail wurde über das Expressformular von traduset.de gesendet";
					...
					$message = sprintf($format, $customerName, $_POST['customerEmail'], $sourceLanguage, $targetLanguage, $_POST['certified']);
...
// Updating widget replacing old instances with new
	...
	public function getEnquiryForm($enquiryFormValues, $enquiryFormErrors, $maxUploadSize)
	{
		...
		$datenschutz = __('Durch Absenden dieses Formulars bestätige ich die Kenntniss der Datenschutzerklärung und bin mit dem Senden und Speichern meiner Daten einverstanden.', 'traduset');
		...
		$form = '<form action="' . $_SERVER['PHP_SELF'] . '" method="post" name="expressEnquiry" enctype="multipart/form-data" id="expressEnquiryForm">
            ...
            <br />
            <span style="font-size: smaller"> Durch Absenden dieses Formulars bestätige ich die Kenntniss der <a href="https://www.traduset.de/datenschutzerklaerung/" target="_blank">Datenschutzerklärung</a> und bin mit dem Senden und Speichern meiner Daten einverstanden.</span>
            <br>
